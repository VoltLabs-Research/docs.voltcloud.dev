...
<Callout type="info">
**TL;DR** — VOLT self-hosting requires a Linux server, Node.js, Docker, MongoDB, Redis, and MinIO. Clone the repo, configure `.env` files, build server and client, set up nginx as a reverse proxy, and verify connectivity.
</Callout>
...
The quickest way to start all services is with Docker Compose. Create a `docker-compose.yml`:
...
<Callout type="info">
If you prefer running containers individually, expand the section below.
</Callout>
...
1. **Check the API** — Open `https://your-domain.com/api/` in a browser. You should see a JSON response confirming the server is running.
2. **Check the client** — Open `https://your-domain.com` in a browser. The VOLT sign-in page should load.
3. **Check MongoDB** — Run `docker logs volt-mongo` and confirm there are no authentication errors.
4. **Check Redis** — Run `docker exec volt-redis redis-cli -a changeme ping` — you should see `PONG`.
5. **Check MinIO** — Open `http://your-server:9001` and sign in with your MinIO credentials. The console should load and show the auto-created buckets after the server starts.
6. **Create an account and connect a cluster** — Sign up, create a team, and connect a cluster to confirm the full pipeline works end to end.

<Callout type="warn">
If any service fails to connect, check `docker ps` to confirm all containers are running, and verify that the ports in your `.env` match the exposed Docker ports.
</Callout>
