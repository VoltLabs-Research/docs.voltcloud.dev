...
Configure the server environment file with the MongoDB, Redis, MinIO, server URL, client URL, secret-key, and encryption settings. The client has no `.env.example`; it is configured at build time through Vite variables — set `VITE_SERVER_ENDPOINT` (the server origin the browser talks to) and, if you proxy the API through a different host, `VITE_PROXY_API_URL`.

Two values are operationally critical:

- `SECRET_KEY` signs sensitive server-side tokens.
- `SSH_ENCRYPTION_KEY` (a legacy key name for `VOLT_SECRET_ENCRYPTION_KEY`) protects credentials at rest, including the SSH access used to install and enroll the daemon on a remote cluster host.
...
A healthy app without a healthy cluster has no execution target: trajectory uploads, analysis runs, notebooks, and container operations all fail.
...
### Proxied container service runbook

If your deployment runs containerized services or internal tools that you reach through VOLT's "Open :PORT" relay, validate at least one proxied service path from end to end. The relay opens short-lived sessions in the `TEAM_CLUSTER_APP_PROXY_PORT_START`–`TEAM_CLUSTER_APP_PROXY_PORT_END` range, so confirm that range is reachable and that the exposure registry, reverse channel, and relay behavior are all healthy. Cluster remote-access terminals and file explorers share the same reverse channel.
...
### Proxied container services never appear
