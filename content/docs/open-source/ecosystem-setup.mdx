...
After a successful run, the compiled plugin binaries are placed in the output directory (default: `scripts/out/`). Each plugin produces a standalone executable that accepts `--input` (path to a LAMMPS dump file), `--output` (path for results), and `--params` (JSON configuration) as command-line arguments, and writes analysis results in MessagePack format to the specified output path.
