...
`analysis.df()` returns the analysis listing rows (one row per timestep) as a DataFrame — the quickest way to a table. `analysis.listings` exposes the same data with extra controls (`columns=`, `to_csv(...)`).
