...
Build GLBs locally from Volt exporter payloads (`AtomisticExporter`, `MeshExporter`, `LineExporter`). It accepts the full artifact file (`.json` or a line-entity-table `.parquet`) or the raw nested payload.
...
`assembler.glb(source, output_path=..., exporter=...)` dispatches to the right exporter automatically; `atomistic_glb`, `mesh_glb`, and `lines_glb` are the per-exporter entry points.
