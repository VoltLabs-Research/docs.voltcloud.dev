...
3. Writes results as MessagePack (`*.msgpack`) files derived from an output base path.
...
json MyPluginService::compute(const LammpsParser::Frame& frame, const std::string& outputBase) {
    ...
    // serializer helpers to write the *.msgpack files under outputBase.
    json result;
...
Every plugin ships a `plugin.json` describing a `{ workflow: { nodes, edges } }` graph. This is not just metadata — it defines the UI form, how the trajectory is iterated, how the binary is invoked, and which `*.msgpack` files are surfaced to the viewer. Edges wire each node's `output` handle to the next node's `input` handle.
...
| `exposure` | A result surfaced in the UI; `results` names a produced `*.msgpack` file. |
...
Each `exposure` node references one of the `*.msgpack` files the binary writes. For `coordination-analysis`, three exposures surface `coordination.msgpack` (the named summary), `atoms.msgpack` (the per-atom data, written in the AtomisticExporter shape and exported to GLB), and `rdf_chart.msgpack` (the RDF chart, exported via `ChartExporter`).

<Callout type="info">
Per-atom data is written to a **separate** `{outputBase}_atoms.msgpack` file (the AtomisticExporter shape), not embedded inside the named summary result.
</Callout>
...
- Validates that the required `*.msgpack` outputs were produced.
...
The plugin writes one or more MessagePack files derived from `output_base`, each declared by an `exposure` node in `plugin.json`. Typically this includes a named summary result and a separate `{outputBase}_atoms.msgpack` holding the per-atom data (AtomisticExporter shape). VOLT reads these files, stores them in MinIO, and makes them available for visualization in the 3D viewer (and for chart/GLB export where the `plugin.json` declares it).
