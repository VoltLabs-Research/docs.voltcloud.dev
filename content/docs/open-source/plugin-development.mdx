...
<Callout type="info">
**TL;DR** — Write a C++ executable that links against CoreToolkit, reads a LAMMPS dump frame via `--input`, runs your analysis algorithm, and writes MessagePack results to `--output`. Build with CMake + Conan, then deploy to any cluster.
</Callout>
