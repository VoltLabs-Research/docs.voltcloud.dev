...
CoreToolkit is organized into seven modules under the `include/volt/` namespace:
...
| `volt.h` | Precompiled header (std lib + spdlog + `boost::dynamic_bitset` + `math/lin_alg.h` + common constants and forward declarations) used as the PCH for the library and all plugins |
...
| `json_utils.h` | JSON serialization helpers (nlohmann/json), including msgpack output |
| `msgpack_atom_writer.h` | Streams per-atom output to a `_atoms.msgpack` file in the canonical `AtomisticExporter` format (used by `volt/plugin/output_serializer.h`) |
...
The plugin-authoring API. These headers wrap the boilerplate of a plugin executable — argument parsing, frame loading, thread control, and result serialization — so an algorithm only needs to implement its `compute()` step.
...
| `output_serializer.h` | `serializePluginOutput()` writes the summary `{outputBase}{suffix}.msgpack` and, when a bucket resolver is supplied, a separate `{outputBase}_atoms.msgpack` |
...
Additionally, three libraries are vendored under `dependencies/`. `MWM_CSP` and `Geogram` are compiled as static libraries; `fast_float` is header-only and installed as headers:
...
For GCC/Clang Release builds, CoreToolkit applies HPC-tuned compiler flags (propagated to all consumers via `PUBLIC`):

```
-O3 -mtune=generic -fno-math-errno -fno-trapping-math -ffp-contract=fast
```

CPU-specific tuning is opt-in. Configure with `-DVOLT_ENABLE_NATIVE_OPTIMIZATIONS=ON` (the option defaults to `OFF`) to swap `-mtune=generic` for `-march=native`.
