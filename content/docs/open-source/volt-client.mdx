...
Creates a new `VoltClient` instance scoped to `baseUrl`. The base URL is a positional first argument; configuration is passed in the optional second argument.
...
`createService(config, methods, factory)` takes three arguments:
...
- `factory` — a `ClientFactory`, `(basePath, opts?) => VoltClient`, that creates the scoped clients (decoupling the DSL from any particular HTTP configuration).

It returns a built service whose methods are called directly — `service.method(params)` — with no second application.
...
`ApiError` exposes a string `code` (e.g. `'Network::Timeout'`, `'Auth::Unauthorized'`) as its primary discriminator, alongside an optional HTTP `status` and `message`. It also provides `getFriendlyMessage()`, `isPermissionDenied()`, and the static `ApiError.isRBACError(err)` helper, so consumers can branch on error codes rather than HTTP status alone.
