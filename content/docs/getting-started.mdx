...
A **Volt desktop launcher** is also available. It is an Electron application that downloads the stack sources from the project's GitHub releases and runs the full local stack (MongoDB, Redis, MinIO, volt-server, volt-client) via Docker Compose on your machine. Installers are built with electron-builder and are available for macOS (dmg), Windows (NSIS exe), and Linux (AppImage) from the project's GitHub releases page.
