...
The primary result surfaced in the UI ("Dislocation Lines"), exported to GLB via the generic
`LineExporter`. A standard VOLT **line entity table** (one row per reconstructed,
PBC-clipped line): fixed columns `id` and `points`, every other column is a per-line
property that VOLT discovers, queries and styles generically.
