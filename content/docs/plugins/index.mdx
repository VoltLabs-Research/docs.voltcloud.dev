...
If you open the resulting export in VOLT, each top-level key inside `AtomisticExporter` becomes a separate colored group in the viewer, and `cluster_id` is also available as a per-atom property.
...
You can download the complete example plugin here: [atomistic-exporter-clusters.zip](/assets/plugins/examples/atomistic-exporter-clusters.zip). This example shows how to export grouped atoms using `AtomisticExporter`, and the ZIP is already packaged as an importable plugin with its own `plugin.json` plus the binary ZIP expected by the `Entrypoint` node.
...
The next image explains how the `Exposure` and `Export` nodes should be configured for this workflow.
...
The Python code below writes a MessagePack file with `main_listing`, `sub_listings`, `per-atom-properties`, and a grouped `AtomisticExporter` payload. The keys `Cluster 0`, `Cluster 1`, and `Cluster 2` are the grouped export object consumed by VOLT, while `cluster_id` is attached back to each atom as a per-atom property.
