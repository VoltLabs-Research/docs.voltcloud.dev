...
The script runs in four phases: it reads the current dump and finds the `ITEM: ATOMS` section, validates that the required columns exist, assigns each atom to an example cluster, and builds a single Apache Parquet payload containing listings, `per-atom-properties`, and the grouped `AtomisticExporter` data.
...
An exposure file is a Apache Parquet object containing one or more of these keys:
...
Here the important node is **Exposure**, because it tells VOLT to ingest the Apache Parquet file written by the program and render the listing output in the UI.
...
The Python code below writes the Apache Parquet file consumed by that **Exposure** node.
