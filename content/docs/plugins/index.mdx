...
<Callout type="info">
**TL;DR** — A VOLT plugin is a node-based workflow that runs analysis binaries on your trajectory frames. Plugins are automatically deployed when you create a team. Results are stored as MessagePack files on your cluster.
</Callout>
...
This set of plugins implements the [Foundational Algorithms](/docs/open-source#foundational-algorithms) from the VOLT open-source ecosystem.
...
The 3D model construction for the Dislocation, Mesh, and Atomistic exporters is handled by [SpatialAssembler](/docs/open-source/spatial-assembler) — a tool developed by VOLT Labs.
...
After a plugin finishes, the exported `.msgpack` files and any generated 3D models are uploaded to MinIO on the team's cluster. VOLT itself does not store analysis data — everything lives on your infrastructure.

<Callout type="info">
For details on how clusters store and serve data, see [Clusters](/docs/modules/clusters).
</Callout>
