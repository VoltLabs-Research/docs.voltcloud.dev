...
In addition to the parquet files, the entrypoint emits plain-text artifacts: `{outputBase}_annotated.dump` (per-atom structure-annotated LAMMPS dump) and the cluster graph tables `{outputBase}_clusters.table` and `{outputBase}_cluster_transitions.table`, which are required inputs for OpenDXA and other DXA-compatible plugins.
