...
Polyhedral Template Matching (PTM) identifies the local crystalline structure around each atom by matching its neighbor shell against a set of reference templates. Besides the structure type, it recovers per-atom orientation, RMSD, interatomic distance and deformation gradient, and emits the reconstructed cluster graph that downstream DXA-compatible tools (OpenDXA, LineReconstructionDXA, ElasticStrain) consume.
...
### `{outputBase}_ptm_analysis.msgpack`

Summary record for the frame. `sub_listings` is emitted as an empty object and `per-atom-properties` as an empty array; the per-atom data lives in `{outputBase}_atoms.msgpack`. The `input_dump`, `clusters_table` and `cluster_transitions_table` fields are filesystem paths to the artifacts written alongside this record.
...
In addition to the msgpack files, the entrypoint emits plain-text artifacts: `{outputBase}_annotated.dump` (per-atom structure-annotated LAMMPS dump) and the cluster graph tables `{outputBase}_clusters.table` and `{outputBase}_cluster_transitions.table`, which are required inputs for OpenDXA and other DXA-compatible plugins.
