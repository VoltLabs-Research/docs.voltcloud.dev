...
The notebook image is built on the Jupyter `scipy-notebook` base (which provides `numpy`, `scipy`, `matplotlib`, and `pandas`) and adds `jupyterlab`, `voltsdk` (2.0.0), `ovito`, `ase`, `pymatgen`, `MDAnalysis`, `matminer`, `vtk`, `minio`, and `parquet`.
