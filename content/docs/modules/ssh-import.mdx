...
<Callout type="info">
**TL;DR** — Connect to remote servers via SSH, browse their file system, and import LAMMPS files directly into VOLT. The entire download-parse-render pipeline runs automatically on your cluster.
</Callout>
...
## Task: Import a trajectory from a remote server

1. Go to the **SSH Import** section from the sidebar.
2. Click **Add Connection** and fill in the host, port, username, and password for the remote server.
3. Click **Test** to verify the connection succeeds.
4. Click **Browse** to open the remote file explorer.
5. Navigate to the directory containing your dump file and select it.
6. Click **Import** to start the automated pipeline.
7. Open the **Jobs** panel to monitor download, parsing, and upload progress.

**Expected outcome:** The trajectory appears in the trajectory listing with full metadata, frames, and 3D models ready for viewing.
