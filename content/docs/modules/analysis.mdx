...
An analysis is a persisted run of a plugin workflow against a trajectory, stored as a record with linked trajectory context, status, progress, and result exposures. Runs can be revisited to compare results, retry failed frames, inspect listings, or load a generated 3D artifact into the viewer.
...
Most of these start as Apache Parquet data and are transformed into the representation the UI requires.
...
The Jobs panel is team-wide, reporting queue activity across all workspace members in real time.
